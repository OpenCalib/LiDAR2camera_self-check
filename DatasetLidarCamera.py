...
class DatasetLidarCameraKittiOdometry(Dataset):
        ...
        if split == 'val' or split == 'test':
            ...
            if os.path.exists(val_RT_file):
                ...
                for i in range(len(self.all_files)):
                    ...
                    #     transl_z = np.random.uniform(-0.1, 0.1)   
                    #     label = 1              
                    val_RT_file.writerow([str(self.all_files[i][:6]),str(self.all_files[i][:-1]), rotx, roty, rotz, transl_x, transl_y, transl_z, label])
                    self.val_RT.append([str(self.all_files[i][:6]), str(self.all_files[i][:-1]), float(rotx), float(roty), float(rotz), float(transl_x), float(transl_y), float(transl_z), int(label)])                        
    ...
    def generate_initial_transformation(self, data_seed):
        max_angle = self.max_r
        if data_seed > 5: # negative samples have 
            while True:
                rotz_seed = random.randint(1,2)
                roty_seed = random.randint(1,2)   
                rotx_seed = random.randint(1,2)  
                transl_x_seed = random.randint(1,2)   
                transl_y_seed = random.randint(1,2)   
                transl_z_seed = random.randint(1,2)     
                if rotz_seed==2 and roty_seed==2 and rotx_seed==2 and transl_x_seed==2 and transl_y_seed==2 and transl_z_seed==2 :
                    continue
                else: 
                    break
            if rotz_seed > 1:
        ...
        return rotz,roty,rotx,transl_x,transl_y,transl_z,label
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train':
            ...
            roty = initial_RT[1]
            rotx = initial_RT[0]
            transl_x = initial_RT[3]
            transl_y = initial_RT[4]
            transl_z = initial_RT[5]  
            label =  initial_RT[6]    
        ...
        # # 正样本也需要R，T，但都设置为0
        # R_pos = mathutils.Euler((0.0, 0.0, 0.0), 'XYZ')
        # T_pos = mathutils.Vector((0.0, 0.0, 0.0))

        # R_pos, T_pos = invert_pose(R_pos, T_pos)
        # R_pos, T_pos = torch.tensor(R_pos), torch.tensor(T_pos)        
        ...
        if self.split == 'test':
            sample = {'rgb': img, 'point_cloud': pc_in, 'calib': calib,
                      ...
                      'rgb_name': rgb_name + '.png', 'item': item, 'extrin': RT,'label':label,
                      'initial_RT': initial_RT}
        else:
            sample = {'rgb': img, 'point_cloud': pc_in, 'calib': calib,
                      'tr_error': T, 'rot_error': R, 
                    #   'tr_pos': T_pos, 'rot_pos': R_pos, 
                      'label_pos': 0, 'label_nag': 1, 'label':label,
                      'seq': int(seq),
